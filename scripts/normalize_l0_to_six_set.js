...
function mapL0(oldL0, amount) {
  if (!oldL0) return amount < 0 ? "OPERATING EXPENSE" : "INCOME";

  const up = String(oldL0).trim().toUpperCase();
  if (ALLOWED_L0.has(up)) return up;

  const v = String(oldL0).trim().toLowerCase();

  if (v === "income") return "INCOME";
  if (v === "expense" || v === "expenses") return "OPERATING EXPENSE";
  if (v.includes("operating")) return "OPERATING EXPENSE";
  if (v.includes("income")) return "INCOME";
  if (v.includes("asset")) return "ASSET";
  if (v.includes("liability")) return "LIABILITY";
  if (v.includes("equity")) return "EQUITY";
