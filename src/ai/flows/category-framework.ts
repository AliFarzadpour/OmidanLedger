
export const MasterCategoryFramework = `
1. Income (Revenue)
  1.1 Operating Income
    - Product Sales
    - Service Income
    - Subscription Income
    - Consulting Income
    - Rental Income
    - Maintenance / Support Income
    - Usage-Based Revenue
    - Membership Fees
    - Ecommerce Sales
    - Marketplace Sales (Amazon, Etsy, Shopify)
  1.2 Non-Operating Income
    - Interest Income
    - Dividend Income
    - Realized Gains
    - Unrealized Gains
    - Asset Sale Income
    - Refunds / Rebates Received
  1.3 Contra-Income
    - Discounts Given
    - Sales Returns
    - Chargebacks
    - Refunds to Customers

2. Cost of Goods Sold (COGS)
  2.1 Product-Based COGS
    - Inventory Purchases
    - Freight / Shipping for Inventory
    - Packaging
    - Warehousing Fees
    - Customs / Import Duties
    - Manufacturing Costs
  2.2 Service-Based COGS
    - Subcontractor / Freelancer Payments
    - Supplies Used in Service Delivery
  2.3 Ecommerce COGS
    - Amazon FBA Fees
    - Shopify/Stripe Processing Fees
    - Shipping Labels (USPS/UPS/FedEx)
    - Marketplace Commissions

3. Operating Expenses
  3.1 General & Administrative
    - Office Rent
    - Utilities (Electricity, Water, Gas)
    - Internet / WiFi
    - Office Supplies
    - Postage & Shipping (non-inventory)
    - Dues & Subscriptions
    - Bank Fees
    - Credit Card Interest
    - Telephone / Mobile
    - Printing / Stationery
    - Repairs & Maintenance
    - Furniture & Equipment (expensed)
    - Cleaning Services
    - Storage Unit Fees
  3.2 Marketing & Advertising
    - Digital Advertising (Google, Facebook, etc.)
    - SEO Services
    - Website Hosting & Domains
    - Graphic Design
    - Influencer Payments
    - Promotional Materials
    - Sponsorships
    - Events / Trade Shows
  3.3 Payroll & Employee Expenses
    - Salaries & Wages
    - Payroll Taxes
    - Employee Benefits (Health, Vision, Dental)
    - Employee Reimbursements
    - Recruitment Costs
  3.4 Professional Services
    - Accounting & Bookkeeping
    - Legal Fees
    - Business Consulting Fees
    - Tax Preparation
    - Compliance Fees
    - Registered Agent Fees
    - Contractor Payments (non-COGS)
  3.5 Technology & Software
    - Software & SaaS Subscriptions (e.g., CRM, Project Mgmt)
    - Cloud Hosting (AWS, GCP, Firebase)
    - API Services
    - System Integrations
  3.6 Vehicle & Travel
    - Fuel
    - Auto Repair/Maintenance
    - Insurance (Auto)
    - Parking & Tolls
    - Business Travel (Flights)
    - Hotels
    - Meals (Travel)
    - Local Transportation (Uber, Taxi, Lyft)
  3.7 Insurance
    - General Liability
    - Workers' Compensation
    - Professional Liability (E&O)
    - Cybersecurity Insurance
  3.8 Taxes & Licensing
    - Business Licenses
    - Franchise Tax
    - State/Local Taxes
    - Sales Tax Paid
    - Penalties & Fines (if applicable)

4. Other Expenses
  - Interest Expense
  - Depreciation
  - Amortization
  - Equipment Purchases (CapEx)
  - Realized Losses
  - Unrealized Losses
  - Bad Debt Expense

5. Balance Sheet Categories (AI should be aware)
  (Not always directly categorized from transactions but needed for bookkeeping accuracy)
  5.1 Assets
    - Bank Accounts
    - Cash on Hand
    - Accounts Receivable
    - Prepaid Expenses
    - Inventory
    - Fixed Assets
    - Accumulated Depreciation
    - Security Deposits Held
  5.2 Liabilities
    - Accounts Payable
    - Credit Cards
    - Loans Payable
    - Lines of Credit
    - Payroll Liabilities
    - Sales Tax Payable
    - Customer Deposits / Deferred Revenue
  5.3 Equity
    - Owner’s Draw
    - Owner’s Contribution
    - Retained Earnings
    - Partner Equity

6. Special Handling Categories
  6.1 Needs Review
    - Needs Review - Split Transaction
    - Needs Review - Unclear Description
`;
