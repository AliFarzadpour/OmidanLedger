...
function resolveRentDueForMonth(opts: {
  ...
  const tenantRent =
    ...
    toNum(monthTenant?.rent) ||
    toNum(monthTenant?.monthlyRent);
  ...
  const propRent =
    toNum(property?.financials?.targetRent) ||
    toNum(property?.financials?.rent) ||
    toNum(property?.targetRent);
...
function parseMonthKeyToDate(monthKey: string): Date {
  return parseISO(`${monthKey}-02`); // avoid timezone issues
}
...
// Normalize L0 for KPI classification
function normalizeL0(tx: any): string {
  const raw = String(tx?.categoryHierarchy?.l0 || tx?.primaryCategory || '')
    .toUpperCase()
    .trim();
...
const TenantRow = ({
  ...
  const rentDue = resolveRentDueForMonth({
    monthTenant: tenant,
    property,
    date: viewingDate,
  });
    ...
    <div className="flex justify-between items-center border p-3 rounded-lg bg-slate-50/50">
      ...
      <div className="text-right hidden sm:block">
        ...
        <p className="text-xs text-muted-foreground">
          Lease ends: {tenant.leaseEnd || 'N/A'}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <Button
          variant="ghost"
          size="icon"
          onClick={() => onOpenLease(tenant)}
          title="Auto-Draft Lease"
        >
...
function LeaseAgentModal({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast({
        title: 'Lease Saved!',
        description: 'The lease has been generated and saved to your documents tab.',
      });
    } catch (error: any) {
        ...
        <div className="space-y-4 py-4">
          <div className="bg-slate-50 p-4 rounded-lg border">
            ...
            <p className="text-sm text-slate-600 mt-1">
              I will generate a Texas-compliant lease for {tenant.firstName} {tenant.lastName} and
              save it to this property's documents tab.
            </p>
          </div>
          <div>
            <h4 className="font-semibold text-slate-800">Legal Disclaimer</h4>
            <p className="text-xs text-muted-foreground mt-1">
              This document was generated by an AI assistant. It is not a substitute for legal
              advice from a qualified attorney. Please review carefully before signing.
            </p>
          </div>
        </div>
              ...
              {documents.map((docItem: any) => (
                <div key={docItem.id} className="flex items-start justify-between p-3 bg-slate-50 border rounded-md">
                  <div>
                    ...
                    {docItem.description && (
                      <p className="text-sm text-slate-600 mt-2 pl-2 border-l-2 border-slate-200">
                        {docItem.description}
                      </p>
                    )}
                  </div>
...
export function PropertyDashboardSFH({ property, onUpdate }: { property: any; onUpdate: () => void }) {
  console.log('✅ USING src/components/dashboard/PropertyDashboardSFH.tsx (FULL FILE FIX)');
  ...
  // ✅ KPI tx list we CONTROL (always an array)
  const [kpiTxList, setKpiTxList] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setKpiTxList(list);
          console.log('✅ KPI TX FETCH', { count: list.length, first: list[0] });
        }
      } catch (e) {
        console.error('❌ KPI TX FETCH FAILED:', e);
        if (!cancelled) setKpiTxList([]);
      } finally {
  ...
  const { noi, cashFlow, dscr, economicOccupancy, breakEvenRent, rentalIncome, potentialRent, verdict } =
    useMemo(() => {
      ...
      const txList = Array.isArray(kpiTxList) ? kpiTxList : [];
      ...
      // ✅ Operating expenses (OPERATING EXPENSE only)
      const operatingExpensesValue = monthlyTxs
        .filter((tx: any) => normalizeL0(tx) === 'OPERATING EXPENSE')
        .reduce((sum: number, tx: any) => sum + Math.abs(toNum(tx?.amount)), 0);
    ...
    }, [kpiTxList, property, selectedMonthDate, selectedMonthKey]);
